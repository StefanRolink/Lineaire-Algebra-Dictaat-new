...
{lijn \RT}  
{ $  \vectwee{x}{y} \  = \ \vectweesnel{b} \  + \  \lambda \ \vectweesnel{r} $ 
	\qquad  \qquad of: 
	$ \vec{x}  =   \vec{b}  + \  \lambda \ \vec{r} $} 

Met  $ \vec{x}  =  \vectwee{x}{y} $ en 
...
{Een voorbeeld van een vectorvoorstelling is:  
	$ m: \vectwee{x}{y} \ = \ \vectwee{-5}{0} \  + \ \lambda \ \vectwee{0}{-1}. $\\  Zie figuur  \ref{fig:lijn2d_new}. 
...
\mybv[vlak \RD]
{Een voorbeeld van een vergelijking van een vlak $ V $ is: $ 2x-3y+7z = -5$ }\\  \\
...
\subsubsection{Normaal van lijn a.h.v. lineaire verglijking}
Voor het normaal bepalen van een lijn als we een lineaire verglijking hebben, is een stuk minder rekenwerk. Deze kunnen we namelijk gewoon aflezen. \\

\mybv[normaal lijn]{In het algemeneen geldt voor een linaire vergelijking $ax+by = c$ dat de normaal ervan is: $\vec{n} = \vectwee{a}{b} $. Dus ter voorbeeld, is de vergelijking $3x-4y=-7$, dan is het normaal van die lijn: $\vec{n} = \vectwee{3}{-4} $.}
...
\mybv[normaal vlak]
{Bijv. als $ V: 7x + 3y - z = -20 $ \qquad dan is $ \vec{n} = \vecdrie{7}{3}{-1} $ de normaalvector van $V$. Let wel op: Alle $x$-, $y$- en $z$-waardes moeten dus aan dezelfde kant van het $=$-teken staan. \\}
...
\begin{enumerate}
  ...
  \item   Bereken de afstand tussen  $  Q = (-1,-4,-1)  $ en 
    ...
    $ V: \ \vecdrie{x}{y}{z} \ =  \ \lambda \ \vecdrie{4}{2}{4} + \mu \  \vecdrie{7}{-1}{-3} $	
\end{enumerate}
