...
\figuur[0.55]{Mandelbrot}{Voorbeeld van een fractal, de Mandelbrotverzameling (Bron: \href{https://en.wikipedia.org/wiki/Mandelbrot_set}{Wikipedia}). Tip: Martin Molema kan je over dergelijke figuren van alles vertellen en laten zien. }
...
\mydef[We definiëren een complex getal $z$ als:] {getal in $\mathbb{C}$}{   $  z = a + bi $ \quad met $a$ en $b$ reële getallen}
\mybv[getal in $\mathbb{C}$]{Bijvoorbeeld, $ z = 2+3i  $ is een complex getal. Complexe getallen ($\mathbb{C}$) kun je zien als 2-dimensionale getallen, zie ook figuur \ref{fig:lijnreeel}. }
\begin{figure}[h!]
    \centering
    \begin{tikzpicture}
      \def\R{2.4}
      ...
      \draw[vector, blue!50!black, thick] (O) -- (R) node[above right=-3] {$z=a+bi$};
    \end{tikzpicture}
...
\mydef[Een quaternion is:] {quaternion }{ $ q = a +bi + cj + dk $ \quad Waarbij $a, b, c, d$ \textbf{\textit{reëel}} zijn, en $i, j, k$  \textbf{\textit{imaginair}}\\ \\ 
   Je zou Quarternionen dus kunnen zien als vier-dimensionale getallen, al is dat wat moeilijk grafisch voor te stellen. Bij Quaternionen voldoen $i$, $j$, $k$ zich aan de volgende regels:}
...
\mybv[quaternion]{Ter voorbeeld: $ q_1 = 2 - i  + 2j - 3k $ is een quaternion, net als $ q_2 = - j + k $.}
...
Je zoekt \textit{eerst} in de $ 1^e $ kolom (\blu{blauw} ) welke van $i$, $j$ of $k$ je nodig hebt en \textit{daarna} het volgende  imaginaire getal in de $ 1^e $ rij (\red{rood} ).  Op het kruispunt staat het resultaat van de vermenigvuldiging (de volgorde is belangrijk omdat bv $  i\cdot k = -j $ maar $ k\cdot i = j  $ !) \\ \\
...
\mydef[ ]{geconjugeerd}{Als $ q = a +bi + cj + dk $  \quad dan is: \quad	$ q^* = a - bi - cj - dk $ }\\ \\
We doen even een paar voorbeeldjes: \\
\mybv[geconjugeerd]{Stel $ q = 1 - 2i + 3j + k $ dan is $ q^* = 1 + 2i - 3j - k $ \\ Als $ q =   i - 3j  $  dan is $ q^* = -i + 3j  $}
...
\mydef{puntquaternion}
{Als P = (x, y, z)  in \RD dan is\\
	het puntquaternion dat er bij hoort: \quad	$ p = xi+yj+zk $  
}\\ 
\mybv[puntquaternion]{	Stel $ P = (1, 3, -2) $  \ \  dan is $ p =0 + 1\cdot i+3j-2k \ = i+3j-2k $ \\ het puntquaternion dat er bij hoort
}\\
Voor het berekenen van een rotatie in \RD geldt de volgende formule: 
\mydef{quaternionrotatie}{Als p een punt en $  q_r $ een rotatie is in \RD dan is:\\
