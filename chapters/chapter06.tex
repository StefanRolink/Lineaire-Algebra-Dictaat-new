...
\mybv[getal in $\mathbb{C}$]{\raggedright Bijvoorbeeld, $z=2+3i$ is een complex getal. Complexe getallen ($\mathbb{C}$) kun je zien als $2$-dimensionale getallen, zie ook figuur \ref{fig:lijncomplex}. }
\begin{figure}[h!]
    \centering
    \begin{tikzpicture}
      \def\R{2.4}
...
We zeggen ook wel dat $z$ bestaat uit een reëel deel $a$ en een imaginair deel $bi$. Je kunt met complexe getallen net zo rekenen als met reële getallen als je maar rekening houdt met $i^2 = -1$.\mybv[imaginair]{\raggedright Bijvoorbeeld:}
\begin{align*}
    (1-i)^2 &= (1-i)\cdot (1-i) \\
...
Met $a$, $b$, $c$ en $d$ kun je gewoon rekenen, omdat het reële getallen zijn. \mybv[quaternion]{\raggedright Ter voorbeeld: $ q_1 = 2 - i  + 2j - 3k $ is een quaternion, net als $ q_2 = - j + k $.}
...
\mybv[quaterniontabel]{\raggedright Als $ q_1 = 6 + 3i - 5j + 2k $ en $ q_2 = 2 + i + 4j - 2k $ wat is dan $ q_1\cdot q_2 $? }
...
\mybv[geconjugeerd]{\raggedright We doen even een paar voorbeeldjes:}
\begin{align*}
...
\mybv[puntquaternion]{\raggedright	Stel: $P = (1, 3, -2)$\ dan is het puntquaternion $p$ dat er bij hoort:}
\[
    p=0 + 1\cdot i+3j-2k\ = i+3j-2k
\]

Voor het berekenen van een rotatie in \RD geldt de volgende formule: \mydeftekst{quaternionrotatie}{Als $p$ een punt en $q_r$ een rotatie is in \RD dan is: het geroteerde punt	$p' = q_r\cdot p.q^*_r$ }

\newpage
\setlength{\abovedisplayskip}{5pt}
\setlength{\belowdisplayskip}{5pt}

\mybv[quaternionrotatie]{\raggedright Bijvoorbeeld:}
...
Daarom hebben we de volgende definitie nodig: \mydeftekst{quaternionrotatie}{Gegeven de eenheidsvector $\hat{v}$ en de  hoek $\alpha $ dan is:	het rotatiequaternion:
\[ 
...
\mybv[quaternionrotatie]{\raggedright Ter voorbeeld:}
...
\mybv[quaternionrotatie]{\raggedright We nemen dezelfde vector en hoek als hierboven.} Stel verder dat we $P=(-1,-1,0)$ willen roteren. Dan is het puntquaternion $p$:
\[p = -i-j\]
